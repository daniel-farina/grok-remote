...
## How this fits grok-remote

This repo's Tailscale (or `--local`) dashboard on **:7910** **stays**. That is
the ACP web UI. Build Remote Agent is a **separate** pairing device: store app
+ GitHub/HTTPS relay. `gbr-agent` already probes `:7910` when this dashboard is
running — discovery only, not a second pair protocol. Do not invent a fourth
pair protocol. Do not replace Tailscale with gbr/1.
